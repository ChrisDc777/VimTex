...
import {
  ...
  AgentCodeLine,
  useAgentCodeTokens,
} from "@/components/beui/agents/agent-code";
...
export interface FileDiffProps {
  ...
  className?: string;
  /**
   * `split` = dual old/new gutters (BEUI default).
   * `unified` = single line number + marker — better for narrow chat.
   */
  layout?: "split" | "unified";
  /**
   * Streaming spinner / “complete” check in the header.
   * Hide for pending Accept/Reject proposals (check ≠ accept).
   */
...
export function FileDiff({
  ...
  const nestedScroll = typeof maxHeight === "number" && maxHeight > 0;
  ...
  return (
    <div
      ...
      aria-busy={streaming}
      className={cn(
        "grid w-full min-w-0 max-w-full overflow-hidden text-sm [grid-template-columns:minmax(0,1fr)]",
        className,
      )}
    >
      <button
        ...
        {showStatusIcon ? (
          <span className="grid size-4 shrink-0 place-items-center text-muted-foreground/60">
            {streaming ? (
              ...
              <Check aria-label="Changes ready" className="size-3.5" />
            )}
          </span>
        ) : null}
        <motion.span
      ...
      <AgentDisclosure
        ...
        open={currentOpen}
      >
        <div className="min-w-0 max-w-full pt-1.5">
          <div className="min-w-0 max-w-full overflow-hidden rounded-xl bg-muted/80">
            <div
              ref={viewportRef}
              ...
              style={{
                maxHeight: nestedScroll ? maxHeight : undefined,
                contain: "inline-size",
              }}
            >
              <div className="w-full min-w-0 max-w-full font-mono text-xs leading-5">
                ...
                {lines.map((line, index) => {
                  ...
                  return (
                    ...
                    >
                      {layout === "unified" ? (
                        <span className="w-8 shrink-0 select-none pr-1 text-right tabular-nums text-muted-foreground/40">
                          {lineNo ?? ""}
                        </span>
                      ) : (
                        <>
                          <span className="w-9 shrink-0 select-none pr-2 text-right tabular-nums text-muted-foreground/40">
                            {line.oldLine}
                          </span>
                          <span className="w-9 shrink-0 select-none pr-2 text-right tabular-nums text-muted-foreground/40">
                            {line.newLine}
                      ...
                      <span
                        className={cn(
                          "w-4 shrink-0 select-none text-center text-muted-foreground/45",
                          type === "added" &&
                            "text-emerald-600 dark:text-emerald-400",
                          type === "removed" &&
                            "text-rose-600 dark:text-rose-400",
                      ...
                      <span className="w-0 min-w-0 flex-1 overflow-hidden px-1.5">
                        <AgentCodeLine
                          code={line.content}
                          tokens={tokens?.[index]}
                          className="block w-full max-w-full whitespace-pre-wrap break-all [overflow-wrap:anywhere]"
                        />
                      </span>
                    </div>
                  );
