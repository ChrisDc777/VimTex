...
export function useAgentCodeTokens(
  ...
  const skip = language === "text" || !code;
  const cached = skip ? undefined : tokenCache.get(key);
...
export function AgentCodeLine({
  ...
  return (
    <span className={cn("min-w-0 break-all [overflow-wrap:anywhere]", className)}>
      {tokens
        ? tokens.map((token) => (
            <span
              ...
              className="break-all text-[var(--agent-code-light)] dark:text-[var(--agent-code-dark)]"
            >
              {token.content}
            </span>
          ))
