...
Phased delivery for [ChrisDc777/VimTex](https://github.com/ChrisDc777/VimTex). **Classic Collaborative remains default.**

## Milestones overview

| Milestone | Theme | Target | Exit criteria |
|-----------|-------|--------|---------------|
| **M0** | Foundation + UI convergence | 2026-Q3 | Classic default, shared layer, CI, truthful room semantics |
...
| P0 | Classic shell as default on integration branch | L | workspace layer |
| P1 | Quiet Craft optional shell toggle | M | workspace layer |
...
**M0 exit:** Two browsers edit same room on Classic default; refresh behavior documented; CI green.
...
## Wave 1 proposal (for founder review)

**Theme:** Make Classic trustworthy and shippable without feature sprawl.
...
4. **Workspace extraction** — `useDocumentBuffer`, decouple note state from Yjs double-fire
5. **Classic default branch** — restore collab-first shell on fork integration branch
6. **Cherry-pick wins** — render-note bare math, API limits, CI from `a9e090a`
7. **Collab E2E** — two Playwright contexts, same room
8. **Share polish** — copy toast, visible URL on failure

**Explicitly not in Wave 1:** accounts, billing, Quiet Craft default, PDF, full AI diff UI, Redis/Hocuspocus.

**Wave 2 (after Wave 1 merge):** Quiet Craft toggle, onboarding, standard editing mode, templates, AI accept/reject.
