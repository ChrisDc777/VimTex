...
describe("ai-room-prefs", () => {
  ...
  it("nearest preset snaps to Precise/Balanced/Creative", () => {
    assert.equal(nearestAiTemperaturePreset(0.25), 0.2);
    assert.equal(nearestAiTemperaturePreset(0.5), 0.4);
    assert.equal(nearestAiTemperaturePreset(0.9), 0.8);
  });
});
