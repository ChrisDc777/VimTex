...
`;
  ...
  it("recovers orphan FIND/THEN without @@@PATCH wrapper", () => {
...
`;
    const block = extractPatchBlock(raw);
    assert.ok(block);
    assert.equal(block.before.trim(), "Here is the edit.");
    const proposal = parsePatchBody(block.body);
    assert.ok(proposal);
    assert.equal(proposal.hunks[0].find, "formula");
    assert.equal(proposal.hunks[0].then, "something");
  });
});
