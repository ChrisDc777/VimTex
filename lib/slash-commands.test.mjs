...
test("custom slash commands merge and pass enabled filter", () => {
  assert.equal(normalizeCustomSlashId("Outline"), "outline");
  assert.equal(normalizeCustomSlashId("explain"), null);

  const pool = mergeSlashCommands(SLASH_COMMANDS, [
    {
      id: "outline",
      title: "Outline",
      hint: "Custom",
      instruction: "Write a short outline of the note.",
    },
  ]);
  assert.ok(pool.some((c) => c.id === "outline" && c.custom));

  const menu = filterSlashCommands("", pool, {
    enabledIds: DEFAULT_ENABLED_SLASH_IDS,
  });
  assert.ok(menu.some((c) => c.id === "outline"));
  assert.ok(menu.some((c) => c.id === "explain"));
});
