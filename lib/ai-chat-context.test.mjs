...
test("packAiChatContext includes Level C aux for Studio", () => {
  const packed = packAiChatContext({
    text: "doc",
    includeSelectionContext: true,
    includeAuxiliaryContext: true,
    diagnostics: "L1:1  err",
    outline: "L1 \\section{Hi}",
    citations: "knuth (article)",
  });
  assert.equal(packed.diagnostics, "L1:1  err");
  assert.equal(packed.outline, "L1 \\section{Hi}");
  assert.equal(packed.citations, "knuth (article)");
});

test("packAiChatContext omits Level C aux for Forge", () => {
  const packed = packAiChatContext({
    text: "doc",
    includeSelectionContext: false,
    includeAuxiliaryContext: false,
    diagnostics: "L1:1  err",
    outline: "L1 \\section{Hi}",
    citations: "knuth (article)",
  });
  assert.equal(packed.diagnostics, undefined);
  assert.equal(packed.outline, undefined);
  assert.equal(packed.citations, undefined);
});
