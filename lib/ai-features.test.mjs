...
describe("ai-features gate", () => {
  ...
  it("enables equation-scoped AI on Studio only", () => {
    assert.equal(aiFeatureEnabled("studio", "equationScopedAi"), true);
    assert.equal(aiFeatureEnabled("forge", "equationScopedAi"), false);
  });

  it("enables derivation coach on Studio and Forge", () => {
    assert.equal(aiFeatureEnabled("studio", "derivationCoach"), true);
    assert.equal(aiFeatureEnabled("forge", "derivationCoach"), true);
  });
