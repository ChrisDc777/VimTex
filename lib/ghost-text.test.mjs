...
test("suggests $ only immediately after an opener $", () => {
  assert.equal(suggestMathCloser("hello $", 7)?.text, "$");
  // Mid-math with content after opener should not spam ghost $
  assert.equal(suggestMathCloser("hello $x^2", 10), null);
});
