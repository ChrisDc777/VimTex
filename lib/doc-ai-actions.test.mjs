...
import { filterSlashCommands, SLASH_COMMANDS, DEFAULT_ENABLED_SLASH_IDS } from "./slash-commands.ts";
...
test("chat slash menu keeps core commands without templates", () => {
  const core = filterSlashCommands("", SLASH_COMMANDS, {
    includeTemplates: true,
    enabledIds: DEFAULT_ENABLED_SLASH_IDS,
  });
  const noTpl = filterSlashCommands("", SLASH_COMMANDS, {
    includeTemplates: false,
    enabledIds: [...DEFAULT_ENABLED_SLASH_IDS, "letter"],
  });
  assert.ok(core.some((c) => c.id === "explain"));
  assert.ok(noTpl.some((c) => c.id === "explain"));
  assert.ok(!core.some((c) => c.id === "letter"));
