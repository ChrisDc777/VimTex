import assert from "node:assert/strict";
import { describe, it } from "node:test";
import { AiReviewStore } from "./ai-review-store.ts";

describe("AiReviewStore", () => {
  it("setPending and clearPending notify listeners", () => {
    const store = new AiReviewStore();
    let ticks = 0;
    store.subscribe(() => {
      ticks += 1;
    });
    store.setPending({
      messageId: "m1",
      before: "a",
      after: "b",
      source: "chat",
      createdAt: 1,
    });
    assert.equal(store.hasPending(), true);
    assert.equal(ticks, 1);
    store.clearPending();
    assert.equal(store.hasPending(), false);
    assert.equal(ticks, 2);
  });

  it("commitAuto keeps undo buffer and clears pending", () => {
    const store = new AiReviewStore();
    const edit = {
      messageId: "m2",
      before: "old",
      after: "new",
      source: "chat",
      createdAt: 2,
    };
    store.setPending(edit);
    store.commitAuto(edit);
    assert.equal(store.getPending(), null);
    assert.equal(store.getSnapshot().outcomes.m2, "auto");
    assert.equal(store.takeLastAuto()?.after, "new");
    assert.equal(store.getSnapshot().lastAuto, null);
  });

  it("reset clears all state", () => {
    const store = new AiReviewStore();
    store.setPending({
      messageId: "m3",
      before: "x",
      after: "y",
      source: "chat",
      createdAt: 3,
    });
    store.markOutcome("m3", "accepted");
    const before = store.getSnapshot();
    store.reset();
    const snap = store.getSnapshot();
    assert.notEqual(snap, before);
    assert.equal(snap.pending, null);
    assert.deepEqual(snap.outcomes, {});
    assert.equal(snap.lastAuto, null);
  });

  it("getSnapshot is referentially stable between emits", () => {
    const store = new AiReviewStore();
    const a = store.getSnapshot();
    const b = store.getSnapshot();
    assert.equal(a, b);
    store.setPending({
      messageId: "m4",
      before: "1",
      after: "2",
      source: "chat",
      createdAt: 4,
    });
    const c = store.getSnapshot();
    assert.notEqual(a, c);
    assert.equal(c, store.getSnapshot());
  });

  it("stores patch kind and hunk metadata on pending", () => {
    const store = new AiReviewStore();
    store.setPending({
      messageId: "m5",
      before: "a b",
      after: "A b",
      source: "chat",
      createdAt: 5,
      kind: "patch",
      hunks: [
        {
          id: "h0",
          find: "a",
          then: "A",
          startOffset: 0,
          endOffset: 1,
          applied: true,
        },
      ],
    });
    const pending = store.getPending();
    assert.equal(pending?.kind, "patch");
    assert.equal(pending?.hunks?.length, 1);
    assert.equal(pending?.hunks?.[0].applied, true);
  });
});
